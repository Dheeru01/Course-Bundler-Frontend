...
export const getAllCourses = (category="", keyword="") => async (dispatch) => {
    try {
      ...
      const { data } = await axios.get(`${server}/courses?keyword=${keyword}&category=${category}`,(category, keyword) ,{headers: {"Content-Type": "multipart/form-data",},withCredentials:true,});
  
      dispatch({type: "getAllCoursesSuccess",payload: data.courses});
    } catch (error) {
      dispatch({type: "getAllCoursesFail",payload: error.response.data.message});
    }
  };
